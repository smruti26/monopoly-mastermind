...
import { createInitialState, PLAYER_COLORS, type Player } from "@/lib/game/engine";
import { setGame } from "@/lib/game/store";
import { Minus, Plus, Play, ArrowLeft } from "lucide-react";
import { Link } from "@tanstack/react-router";

export const Route = createFileRoute("/setup")({
  head: () => ({ meta: [{ title: "Setup — Monopoly Royale" }, { name: "description", content: "Configure players, AI opponents and tokens before starting." }] }),
  component: Setup,
});
...
function Setup() {
  ...
  const [humans, setHumans] = useState<HumanCfg[]>([
    { name: "You", tokenId: "hat" },
  ]);
  ...
  function updateHuman(i: number, patch: Partial<HumanCfg>) {
    const next = humans.map((h, idx) => idx === i ? { ...h, ...patch } : h);
    setHumans(next);
  }
  ...
  function start() {
    const players: Omit<Player, "money"|"position"|"properties"|"inJail"|"jailTurns"|"bankrupt">[] = [];
    humans.forEach((h, i) => {
      const tk = TOKENS.find(t => t.id === h.tokenId)!;
      players.push({
        id: `h${i}`, name: h.name || `Player ${i+1}`, token: tk.id, tokenIcon: tk.icon,
        isAI: false, color: PLAYER_COLORS[i],
      });
    });
    ...
    for (let i = 0; i < aiCount; i++) {
      ...
      players.push({
        id: `a${i}`, name: `${aiNames[i]} AI`, token: tk.id, tokenIcon: tk.icon,
        isAI: true, color: PLAYER_COLORS[humans.length + i],
      });
    }
  ...
  return (
    <main className="min-h-screen px-6 py-10 max-w-4xl mx-auto">
      ...
      <div className="rounded-2xl border border-border bg-card/60 p-2 mb-6 text-sm text-center text-muted-foreground">
        {humanCount} human{humanCount>1?"s":""} · <span className="text-gold">{aiCount} AI opponent{aiCount!==1?"s":""}</span>
      </div>
              ...
              {TOKENS.map(t => {
                ...
                return (
                  <button
                    ...
                    className={`size-14 rounded-xl border text-2xl grid place-items-center transition ${
      ...
      <button
        onClick={start}
        className="w-full sm:w-auto inline-flex items-center justify-center gap-3 px-8 py-4 rounded-full bg-gold text-background font-semibold text-lg shadow-luxe hover:scale-105 transition-transform"
      >
  ...
  return (
    <div className="p-4 rounded-xl border border-border bg-card/60">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => onChange(value-1)} disabled={value<=min} className="size-10 rounded-full border border-border grid place-items-center hover:border-gold disabled:opacity-30"><Minus className="size-4"/></button>
        <span className="text-4xl font-display text-gold">{value}</span>
        <button onClick={() => onChange(value+1)} disabled={value>=max} className="size-10 rounded-full border border-border grid place-items-center hover:border-gold disabled:opacity-30"><Plus className="size-4"/></button>
      </div>
    </div>
  );
}
